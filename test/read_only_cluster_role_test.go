...
func TestReadOnlyGroupClusterRole(t *testing.T) {
	t.Parallel()

	workingDir := "../examples/read-only-group-cluster-role"

	expectedRoleName := "read-only-cluster-role"
	...
	test_structure.RunTestStage(t, "apply", func() {
		...
		roleName := terraform.OutputRequired(t, terraformOptions, "cluster_role_name")
		roleBindingName := terraform.OutputRequired(t, terraformOptions, "cluster_role_binding_name")

		assert.Equal(t, expectedRoleName, roleName)
		assert.Equal(t, expectedRoleBindingName, roleBindingName)
	})

	// Check the k8s cluster ClusterRole and ClusterRoleBinding objects
	test_structure.RunTestStage(t, "k8s_objects", func() {
		kubectlOptions := k8s.NewKubectlOptions("", "", "")

		// Check the ClusterRole
		role := k8s.GetClusterRole(t, kubectlOptions, expectedRoleName)
		...
		for index, element := range apiGroups {
			...
			assert.Equal(t, []string{"get", "list", "watch"}, role.Rules[index].Verbs)
		}

		// Check the RoleBinding.
		roleBinding := getClusterRoleBinding(t, kubectlOptions, expectedRoleBindingName)
